...
class NeRFSystem(LightningModule):
    ...
    def __init__(self, hparams_):
        ...
        if hparams_.use_mask:
            if hparams_.use_depth:
                print("Using ZoeDepth")
                self.implicit_mask = Context_Guided_Network(classes= 1, M= 2, N= 2, input_channel=6)
            else:
    ...
    def forward(self, rays, ts, whole_img, W, H, rgb, rgb_idx, test_blender, args, hw_whole=None,val_mode=False):
        ...
        if self.hparams_.use_mask:
            # Zoedepth inference
            if self.hparams_.use_depth:
                depths = Zoe.infer(whole_img)
                depths_colored = colorize(depths)[..., :3]
                depths_colored = torch.from_numpy(depths_colored).permute(2, 0, 1).unsqueeze(0).to(whole_img.device)
                whole_img = torch.cat([whole_img, depths_colored], dim=1)
        ...
        if (self.global_step + 1) % 5000 == 0 or (self.global_step + 1)==2 :
            ...
            if self.hparams_.use_mask:
                ...
                if 'rgb_fine_random' in results:
                    ...
                    # cat mask, img_gt, and img
                    test = torch.cat([img_gt.cpu(), img, mask], 2).permute(1, 2, 0).numpy()
                    # resize use lancoz
                    test = cv2.resize(test, (320*3, 320), interpolation=cv2.INTER_LANCZOS4)
                    Image.fromarray((test * 255).astype(np.uint8)).save(f'../mask/mask_{self.n}.png')
                    self.n+=1
